...
namespace RectangleIntersection
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                string[] currentRectangle = Console.ReadLine()
                .Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
            ...
            for (int i = 0; i < numberOfChecks; i++)
            {
                ...
                var firstRec = rectangles.First(r => r.iD.Equals(check[0]));
                var secondRec = rectangles.First(r => r.iD.Equals(check[1]));

                //bool res = firstRec.Intersect(secondRec);
                //if (res == true)
                //{
                ...
                result.AppendLine(firstRec.Intersect(secondRec) ? "true" : "false");
            }
